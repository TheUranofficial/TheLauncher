...
public class UIGameModePanel extends UIDataDashboardPanel<GameMode> {
    ...
    public UIGameModePanel(UIDashboard dashboard) {
        ...
        this.play.color(Colors.GREEN).relative(this.editor).xy(0.015f, 0.9f).wh(80, 20);

        this.description = new UIText();
        this.description.relative(this.editor).xy(14, 70).full();

        this.version = new UILabel(IKey.EMPTY, Colors.LIGHTEST_GRAY);
        this.version.relative(this.editor).xy(14, 42).full();

        this.editor.add(this.play, this.description, this.version);
    ...
    private void runMinecraftClient(UIButton button) {
        ...
        try {
            ...
            processBuilder.directory(BBS.getGamePath("profiles/" + data.getId()));
    ...
    protected void fillData(GameMode data) {
        ...
        this.version.setVisible(data != null);

        if (data != null) {
            this.description.text(data.description.get());
            this.version.label = IKey.raw(data.modLoader.get() + " " + data.version.get());
        }
    }
    ...
    public void render(UIContext context) {
        ...
        if (this.data != null) {
            ...
            //context.batcher.textShadow(context.font, this.data.modLoader.get() + " " + this.data.version.get(), 14, 42, Colors.LIGHTEST_GRAY);
